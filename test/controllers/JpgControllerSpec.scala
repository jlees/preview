 ...
 */
class JpgControllerSpec extends Specification {
  
  "JpgController" should {
    
    "render a JPG given a valid request containing a documentContent " in {
      running(FakeApplication()) {
        ...
        val action = controllers.JpgController.document()

        val response = action(request)
        
        status(response) must equalTo(OK)
        contentType(response) must beSome.which(_ == "image/jpeg")
